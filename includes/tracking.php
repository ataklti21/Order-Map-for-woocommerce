...
add_action( 'init', function () {
	add_shortcode( 'wom_customer_tracking', function () {
		...
		echo '<script>(function(){function g(cb){if(!navigator.geolocation){cb(null);return;}navigator.geolocation.getCurrentPosition(function(p){cb({lat:p.coords.latitude,lng:p.coords.longitude});},function(){cb(null);});}function post(u,d){return fetch(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(d)}).then(function(r){return r.json();});}var t=' . wp_json_encode( $token ) . ';var root=' . wp_json_encode( esc_url_raw( rest_url( 'wom/v1' ) ) ) . ';var el=document.getElementById("wom-customer-track");el.innerHTML="<p>Sharing your location helps your driver find you.</p><button id=\"wom-share\">Share my location</button>";document.getElementById("wom-share").addEventListener("click",function(){g(function(pos){if(!pos){alert("Unable to get your location.");return;}post(root+"/track/share",{token:t,lat:pos.lat,lng:pos.lng}).then(function(){el.innerHTML="<p>Thanks! Your location was shared.</p>";});});});})();</script>';
		return ob_get_clean();
	} );
} );
